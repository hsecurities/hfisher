...
CONTAINER="hfisher"
IMAGE="htrtech/hfisher:latest"
IMG_MIRROR="ghcr.io/hsecurities/hfisher:latest"
...
if [[ ! $check_container == $CONTAINER ]]; then
	...
	docker create \
		--interactive --tty \
		--volume ${MOUNT_LOCATION}:/hfisher/auth/ \
		--network host \
		--name "${CONTAINER}" \
		"${IMAGE}"
fi
...
# docker run --rm -ti --network="host" -v ${MOUNT_LOCATION}:/hfisher/auth/ --name "$CONTAINER" "$IMAGE"
